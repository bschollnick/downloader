...
class PluginOne(IPlugin):
    ...
    def login(self, options):
        ...
        search_data = soup.find_all('input', 
                                    {'type':'hidden', 'name':'validate_token'})
        validate_token = search_data[0]["value"]
        search_data = soup.find_all('input', 
                                    {'type':'hidden', 'name':'validate_key'})
        ...
        if data.find('"loggedIn":true') == -1:
            ...
            print data
            sys.exit(1)
        elif data.find('<span class="field_error" rel="password">The password you entered was incorrect.</span>') != -1:
    ...
    def search_for_folders(self, soup_bowl):
        ...
        for x in tv150_tags:
            folder_name = x.find_all("div", {"class":"tv150-tag"})[0].text
            folder_url = x.find_all("a", {'class':'tv150-cover'})[0]["href"]
        ...
        #
        customicons = soup_bowl.find_all('div', {'class':'rs-customicon-cont'})
        for x in customicons:
            folder_url = x.find_all("a", 
                                    {"class":"rs-customicon-link"})[0]["href"]
            try:
                folder_name = x.find_all("a", {'class':''})[0].text
            except IndexError:
        ...
        for x in class_a_tags:
            ...
            folder_name = x.find_all("a", {"class":"a"})[0].text
            folder_url = x.find_all("a", {"class":"a"})[0]["href"]
            found_urls.append((folder_name, folder_url))

            #<div style="line-height:1.3em" class="gl-text" 
            #       collect_rid="20:32170436">
            #<a class="a" href="http://arconius.deviantart.com/gallery/32170436">
            #   Ponies</a>
            #</div>
        return found_urls
...
#        soup = BeautifulSoup(current_webpage)
        ...
        status = self.download_gallery_images(gallery_url,#options.url_to_fetch,
                                              download_path, 
                                              options,
                                              status)
...
#        links = subfolder.find_all('a', href=True)
        ...
        for x in links:
            ...
            if os.path.exists(\
                ...
                status.add_skipped(filename=file_to_download,
                                   options=options)
                current_skips += 1
                if options.skiplimit != 0 and current_skips >= options.skiplimit:
                    return status
            else:                                       
